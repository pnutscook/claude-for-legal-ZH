...
>
> 然后："设置从您的回答中构建全新的职业画像。它不读取您的个人 Codex 历史、其他对话或您主目录的PRACTICE.md。如果本对话中此前出现过相关内容（例如您提到了您的公司），我会在使用前询问。除非您输入或批准，任何内容都不会进入您的配置。"
>
...
对于显示为未连接的连接器，告诉用户如何连接。示例措辞："飞书多维表格未连接。在 Codex 中，将飞书多维表格 MCP 添加到您的配置并按提示授权。此插件可以没有它运行——您将直接粘贴 PRD 和审查文件——但连接后产品上线监控代理可以自动拉取工单。"
...
2. **研究连接器提示。** 说：
   ...
   > "在您的第一个产品上线审查前：连接一个研究工具。没有的话，我会将每条引用标记为未核实——有了的话，我对照现行数据库验证它们。在 Codex 中按技能提示授权，或在配置中添加对应 MCP。"
