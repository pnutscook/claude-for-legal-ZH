...
4. On first run after a plugin update, if a populated PRACTICE.md exists at the old cache path
   (~/.codex/plugins/cache/claude-for-legal-zh/legal-builder-hub/<version>/PRACTICE.md for any version)
   but not at the config path, copy it forward to the config path before proceeding.
5. This file (the one you are reading) is the TEMPLATE. It ships with the plugin and shows the
   structure the config should have. It is replaced on every plugin update. Never write user data here.
6. Before substantive output, apply the internal `legal-builder-hub:quality-gate`; it controls source trust, code review, legal-content currency and consequential actions.
...
**下一步决策树。**在分析、审查、分流或评估之后，以决策树结尾 — 是选项的草稿，而非决定的草稿。律师选择；Codex 展开。格式：
...
插件的职责是让 Codex 在法律工作中**做得更好**，而非将其引离它已知的法律学说。当技能有清单或工作流程时，清单是底线（FLOOR），而非上限。
