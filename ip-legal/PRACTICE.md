...
4. On first run after a plugin update, if a populated PRACTICE.md exists at the old cache path
   (~/.codex/plugins/cache/claude-for-legal-zh/ip-legal/<version>/PRACTICE.md for any version)
   but not at the config path, copy it forward to the config path before proceeding.
5. This file (the one you are reading) is the TEMPLATE. It ships with the plugin and shows the
   structure the config should have. It is replaced on every plugin update. Never write user data here.
6. Before substantive output, apply the internal `ip-legal:quality-gate`; it controls sourcing, factual gaps, escalation and consequential actions.
...
**验证日志。** 记录到 `~/.codex/plugins/config/claude-for-legal-zh/ip-legal/verification-log.md`。
...
**登记册：** `~/.codex/plugins/config/claude-for-legal-zh/ip-legal/portfolio.yaml`
