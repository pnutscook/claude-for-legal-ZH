...
1. 读取 `~/.codex/plugins/config/claude-for-legal-zh/ip-legal/PRACTICE.md`。如包含 `[占位符]`，停止并引导至 `ip-legal:cold-start-interview`。
2. 遵循以下工作流。
...
**事项上下文。** 检查实践级 PRACTICE.md 中的 `## 事项工作区`。如果 `已启用` 为 `✗`，跳过。如果已启用且无活动事项，询问。专利FTO事项特别常见于需要**隔离团队**或**增高**保密级别的事项开启时。尊重事项的保密标记。
