...
**全新的职业画像。** 设置从用户的回答和明确分享的文件构建全新的职业画像。它不读取用户的个人 Codex 历史、不相关的对话或主目录下的 PRACTICE.md。如果当前会话上下文中出现相关内容（例如他们早些提到了公司），在使用前询问——除非用户输入或批准，不要将任何个人信息纳入公司业务实务画像。
...
对显示为未连接的连接器，告知用户如何连接。示例措辞："飞书未连接。在 Codex 中，将飞书 MCP 添加到你的配置并按提示授权。本插件在没有它的情况下也能工作——你将粘贴文件而非拉取——但连接它让文件拉取自动化。"
...
> "在你的第一次尽调提取或决议之前：连接一个研究工具。没有它，我会将每个引用标注为未核实——有了它，我对照最新数据库核实它们。在 Codex 中按技能提示授权，或在配置中添加对应 MCP。"
