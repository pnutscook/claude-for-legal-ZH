...
User-specific configuration for this plugin lives at a version-independent path that survives plugin updates:
  ...
  ~/.codex/plugins/config/claude-for-legal-zh/legal-clinic/PRACTICE.md
...
4. On first run after a plugin update, if a populated PRACTICE.md exists at the old cache path
   (~/.codex/plugins/cache/claude-for-legal-zh/legal-clinic/<version>/PRACTICE.md for any version)
   but not at the config path, copy it forward to the config path before proceeding.
5. This file (the one you are reading) is the TEMPLATE. It ships with the plugin and shows the
   structure the config should have. It is replaced on every plugin update. Never write user data here.
6. Before substantive output, apply the internal `legal-clinic:quality-gate`; it controls sourcing, factual gaps, supervision and consequential actions.
...
**本地规则已收录（Local rules ingested）：** [PLACEHOLDER — 列出文件，或"尚无 — legal-clinic:draft 将使用省级默认并标记"]
...
*`legal-clinic:draft` 知道的起手文件。在 cold-start 时填充；通过
编辑此处或上传模板添加更多。*
...
**下一届导入（Next cohort onboards）：** [PLACEHOLDER — 下次运行 legal-clinic:ramp 的时间]
**离届移交（Departing cohort handoff）：** [PLACEHOLDER — 运行 legal-clinic:semester-handoff 的时间；通常为学期结束前 1-2 周]

---

## Seed documents

*指导老师在 cold-start 时上传的内容。`legal-clinic:ramp` 和 `legal-clinic:draft` 读取这些。设置目标：10-20 项。少于 10 项适用 LIMITED DATA 标记。*
...
|---|---|---|
| 诊所手册（Clinic handbook） | [PLACEHOLDER] | `legal-clinic:ramp` 据此教学 |
| 提交指南（Filing guides） | [PLACEHOLDER] | `legal-clinic:draft` 应用这些 |
| 本地法院规则（Local court rules） | [PLACEHOLDER] | `legal-clinic:draft` 应用这些 |
| 接待表格（Intake form(s)） | [PLACEHOLDER] | `legal-clinic:client-intake` 使用这些 |
...
- 插件命令转交（"接下来运行 other-plugin:other-skill……"）：从交付物中删除；放在单独的审查备注中
- "我读了以下文件……"：删除

交付物应该读起来像指导老师写的。元注释放在头上的审查备注或单独消息中，而非文件中。

**下一步决策树。**在分析、审查、分流或评估之后，以决策树结尾 — 是选项的草稿，而非决定的草稿。律师/指导老师选择；Codex 展开。格式：
...
指导老师可以在 `~/.codex/plugins/config/claude-for-legal-zh/legal-clinic/guides/<practice-area>.md` 撰写按实践领域的指导。面向学生的技能在做实质性工作前阅读该指导。指导控制：
...
**核实日志。**当你或用户核实了一个标记项时，在 `~/.codex/plugins/config/claude-for-legal-zh/legal-clinic/verification-log.md` 中写入一行记录。当出现的标记项已在核实日志中时，审查备注引用该条目。日志是按插件的，不是按事项的。
...
**研究输出特别注意：**`legal-clinic:research-start` 产出线索，而非权威
...
*驱动 `legal-clinic:deadlines`。默认节奏：截止日期前 14、7、3 和 1 天出现预警。逾期截止日期保持标记直到标记完成或明确关闭。*

**预警天数（Warning days）：** [PLACEHOLDER — 默认 14, 7, 3, 1]
**截止日期文件（Deadlines file）：** `~/.codex/plugins/config/claude-for-legal-zh/legal-clinic/deadlines.yaml`（由 `legal-clinic:deadlines --add` 填充）
...
插件的职责是让 Codex 在法律工作中**做得更好**，而非将其引离它已知的法律学说。当技能有清单或工作流程时，清单是底线（FLOOR），而非上限。如果用户的问题涉及清单未覆盖的法律分析，仍然回答问题并注明。在其自身领域中给出比未加载本插件的 Codex 更差答案的插件已经失败了。
