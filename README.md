...
<p align="center">
  <a href="https://github.com/pnutscook/claude-for-legal-ZH/blob/main/LICENSE"><img src="https://img.shields.io/badge/License-Apache%202.0-blue.svg" alt="License"></a>
  <a href="https://github.com/pnutscook/claude-for-legal-ZH"><img src="https://img.shields.io/badge/version-v1.0.0-brightgreen" alt="Version"></a>
...
本仓库现在默认作为 **Codex 本地插件 marketplace** 使用。每个法律领域目录都包含 Codex manifest、技能、MCP 连接器和实践画像模板；旧 `.claude-plugin/` 文件保留为 legacy compatibility，不作为默认安装路径。

## 在 Codex 中安装

在 `~/.codex/config.toml` 中加入本地 marketplace：
...
`source` 指向本仓库根目录；Codex 会读取 `.agents/plugins/marketplace.json`。重启 Codex 后，在插件列表中安装需要的领域插件，然后运行初始化设置：
...
**旧 Claude 兼容。** `.claude-plugin/` 和旧 `CLAUDE.md` 模板仍保留；Codex 默认读取 `.codex-plugin/plugin.json`、`.agents/plugins/marketplace.json` 和 `PRACTICE.md`。冷启动技能会在需要时把旧 Claude 画像迁移到 Codex 配置目录。
...
Codex automation / recipe 参考——`agent.yaml`、leaf-worker 子 Agent、steering 事件示例和各工作流安全说明，详见 **[managed-agent-cookbooks/](./managed-agent-cookbooks)**。这些 cookbooks 是迁移参考，不声明为 Codex 自动运行的后台任务。
...
managed-agent-cookbooks/  # Codex automation/recipe 参考——每个监控型工作流一个目录
  diligence-grid/
  docket-watcher/
  launch-radar/
  reg-monitor/
  renewal-watcher/
scripts/                  # validate-codex.py · validate.py · orchestrate.py · lint-tool-scope.py
.agents/plugins/
  marketplace.json        # Codex 本地 marketplace
.claude-plugin/
  marketplace.json        # 旧 Claude compatibility 注册表
```
...
<插件名>/
  ...
  .claude-plugin/plugin.json # 旧 Claude compatibility manifest
  PRACTICE.md               # Codex 实践画像模板——由 <插件名>:cold-start-interview 填充
  CLAUDE.md                 # 旧 Claude compatibility 模板
  README.md
  skills/                   # Codex 技能文件——以 <插件名>:<技能名> 调用
  agents/                   # automation/recipe 参考（如有）
  hooks/                    # 旧 Claude hooks（Codex manifest 不引用）
```
...
| **技能** | 领域专业知识、惯例和分步方法论，Codex 在相关时自动调用，也可以通过 `commercial-legal:review`、`privacy-legal:dsar-response`、`litigation-legal:claim-chart` 显式触发。 | `<插件名>/skills/<技能名>/SKILL.md` |
| **Agent / recipe** | 定时或事件驱动工作流的参考材料（续签监控、案件进度监控、法规变化监控）。Codex 自动化落地时应按当前环境单独配置。 | `<插件名>/agents/` |
| **实践画像** | 描述你的审查指引、升级规则和内部风格的纯文本 `PRACTICE.md`。所有技能从中读取。 | `~/.codex/plugins/config/claude-for-legal-zh/<插件名>/PRACTICE.md` |
| **连接器** | 将 Codex 与你的数据系统连接的 [MCP 服务器](https://modelcontextprotocol.io/)——合同管理、文档管理、电子取证、检索平台、生产力工具。 | `.mcp.json`（每个插件） |
| **automation cookbook** | `agent.yaml` + 一级子 Agent + steering 示例，用于后续迁移为 Codex automation。 | `managed-agent-cookbooks/<slug>/` |
...
> "客户订阅"标记的连接器需要客户自身的账户和 API 密钥。在各插件的 `.mcp.json` 中配置，或通过 Codex 的 `claude mcp` 进行设置。
...
- **编辑实践画像。** 你的画像位于 `~/.codex/plugins/config/claude-for-legal-zh/<插件名>/PRACTICE.md`。直接编辑以修正小问题——错误的升级阈值、新的集成、政策更新。它在插件更新后保留。
...
- **带入你的审查指引和模板。** 将你的术语、内部风格和品牌模板放入插件的 `PRACTICE.md` 和 `references/`。技能会自动拾取。
...
[![Star History Chart](https://api.star-history.com/svg?repos=pnutscook/claude-for-legal-ZH&type=Date)](https://www.star-history.com/?repos=CSlawyer1985%2Fclaude-for-legal-ZH)
