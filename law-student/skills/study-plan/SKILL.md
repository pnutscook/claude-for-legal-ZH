...
# study-plan
1. 加载 `~/.codex/plugins/config/claude-for-legal-zh/law-student/PRACTICE.md` → 考试类型（客观题/主观题）、考试日期、薄弱科目、每日目标学习时数、培训课程。
2. 加载 `~/.codex/plugins/config/claude-for-legal-zh/law-student/study-plan.yaml`（如存在）。
3. 应用以下框架。
4. 按标志路由：
...
`~/.codex/plugins/config/claude-for-legal-zh/law-student/study-plan.yaml`（如存在）——扩展，不覆盖。
...
如果 `~/.codex/plugins/config/claude-for-legal-zh/law-student/PRACTICE.md` → `培训课程` 是**瑞达**、**厚大**、**众合**或其他结构化培训课程（即不是 `自学` 或 `不适用`），学生已经有了一个培训机构的日程表。本技能的计划必须选择两种角色之一——它不能在培训课程旁边运行一个完整的平行课程而不让学生崩溃。
...
写入 `~/.codex/plugins/config/claude-for-legal-zh/law-student/study-plan.yaml`：
