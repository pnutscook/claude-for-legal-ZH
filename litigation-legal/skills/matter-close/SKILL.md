...
5. 向 `~/.codex/plugins/config/claude-for-legal-zh/litigation-legal/matters/[slug]/history.md` 追加最终条目。
6. 案件保留在 `_log.yaml` 和 `~/.codex/plugins/config/claude-for-legal-zh/litigation-legal/matters/[slug]/` 中——不删除。`litigation-legal:portfolio-status` 从活跃汇总中过滤。
...
- `~/.codex/plugins/config/claude-for-legal-zh/litigation-legal/matters/_log.yaml` —— 找到对应行
- `~/.codex/plugins/config/claude-for-legal-zh/litigation-legal/matters/[slug]/matter.md` —— 参考（登记时上下文）
- `~/.codex/plugins/config/claude-for-legal-zh/litigation-legal/matters/[slug]/history.md` —— 追加目标
...
**在结案（产生后果的行为——案件被归档且停止主动追踪）之前：** 读取 `~/.codex/plugins/config/claude-for-legal-zh/litigation-legal/PRACTICE.md` 中的 `## 使用者`。如果角色是**非律师**：

> 结案具有法律后果——它结束主动追踪，可能影响相关联的证据保全（如适用，另行运行 `litigation-legal:legal-hold --release`），并建立公司依赖的最终记录。您是否已与律师审查过此事？如已审查，继续。如未审查，以下是带去给律师的简要材料：
>
...
### 更新 `~/.codex/plugins/config/claude-for-legal-zh/litigation-legal/matters/_log.yaml`
...
### 向 `~/.codex/plugins/config/claude-for-legal-zh/litigation-legal/matters/[slug]/history.md` 追加最终条目
...
### 触及 `~/.codex/plugins/config/claude-for-legal-zh/litigation-legal/matters/[slug]/matter.md`
