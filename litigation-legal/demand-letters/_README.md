...
After a demand letter is drafted, `demand-draft` assesses materiality (heuristic from house `~/.codex/plugins/config/claude-for-legal-zh/litigation-legal/PRACTICE.md`) and offers to create a matter. If yes, a matter row goes into `matters/_log.yaml` with `source: demand-letter`, and `matters/[matter-slug]/matter.md` links back to this demand-letter's folder.
