...
# dsar-response
1. 加载 `~/.codex/plugins/config/claude-for-legal-zh/privacy-legal/PRACTICE.md` → DSAR 流程（系统清单、验证方式、SLA）。
2. 执行以下工作流。
...
读取 `~/.codex/plugins/config/claude-for-legal-zh/privacy-legal/PRACTICE.md` → `## DSAR 流程`。该节包含：
- 系统清单（用户数据所在的每个位置）
- 身份验证方式
- 回复 SLA
- 谁处理常规请求 vs. 谁处理升级请求
...
按 `~/.codex/plugins/config/claude-for-legal-zh/privacy-legal/PRACTICE.md` 中的验证方式。常见方式：
...
遍历 `~/.codex/plugins/config/claude-for-legal-zh/privacy-legal/PRACTICE.md` 中的系统清单。对每个系统：
...
> **检索连接器预检。** 在输出确认函、实质回复函或内部豁免分析前，检查法律检索连接器是否在本会话中可访问。收集此信息到 PRACTICE.md `## 输出` 下的审核备注中——审核备注放在内部豁免分析和封面备忘录上，不放在对外发给个人信息主体的 DSAR 函件上。如果第1步（权利分类）、第4步（豁免分析）或期限管理检索步骤中没有连接器返回结果，记录在内部审核备注的**来源：**行中——如 `未连接——引用来自训练知识；所主张的豁免、回复期限和延期机制特别容易造假，在向个人信息主体或监管机关主张任何豁免前必须验证`。逐条 `[模型知识 — 需验证]` 标签保持内联。不在输出上方发出独立横幅。
...
**在将任一函件发送给个人信息主体之前：** 读取 `~/.codex/plugins/config/claude-for-legal-zh/privacy-legal/PRACTICE.md` 中的 `## 谁在使用`。如果角色为非律师：
...
> **注意：** 两份 DSAR 函件均为对外发送给个人信息主体的文件。**勿**在两份函件上包含 `~/.codex/plugins/config/claude-for-legal-zh/privacy-legal/PRACTICE.md` `## 输出` 中的工作成果抬头。函件附随的内部笔记、日志和豁免分析是律师工作成果——将这些分开保存并冠以工作成果抬头。
...
按 `~/.codex/plugins/config/claude-for-legal-zh/privacy-legal/PRACTICE.md` → 升级表，以下情况升级：
...
如果 `~/.codex/plugins/config/claude-for-legal-zh/privacy-legal/PRACTICE.md` → `## DSAR 流程` 记录了一个比法定期限更严格的内部 SLA，使用内部 SLA 并注明法定底线。
