...
description: >
  ...
  Runs per the cadence in ~/.codex/plugins/config/claude-for-legal-zh/regulatory-legal/PRACTICE.md. Filters by materiality threshold so the
...
Per `~/.codex/plugins/config/claude-for-legal-zh/regulatory-legal/PRACTICE.md` → Feed configuration → Check cadence. Default weekly; daily if the regulatory environment is active.

## What it does

1. Read `~/.codex/plugins/config/claude-for-legal-zh/regulatory-legal/PRACTICE.md` → watchlist, materiality threshold.
