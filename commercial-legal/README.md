...
商事合同工作流：一般服务、委托、评估及公共部门背景合同审查，供应商协议审查、保密协议分流、SaaS订阅审查、合同续约追踪、审批上报路由以及业务利益方摘要。围绕通过冷启动访谈生成的团队实务画像构建——插件学习的是*您的*合同手册和核验要求，而非通用模板。
...
| 角色 | 主要工作流 |
|---|---|
| **商事合同律师/法务** | 一般服务/委托/评估/供应商协议审查、审批上报路由、业务利益方摘要 |
| **政府项目协作或公益服务人员** | 公共部门背景合同的首轮风险识别、权限/审批/审计材料补件清单 |
...
首次使用时，插件将通过对话形式对您进行访谈——约十分钟——了解您团队的实际运作方式。询问您的合同类型、合同手册立场、一般服务成果/付款要求、公共部门权限与审计材料要求（如适用）、审批上报规则以及让您在材料到桌时头疼的事项。然后要求您提供5-10份近期已签署协议（越多越好，20份能呈现更清晰的模式），以便实地观察您的立场。

将学习到的内容写入 `~/.codex/plugins/config/claude-for-legal-zh/commercial-legal/PRACTICE.md` —— 一份关于您团队的通俗英语文档，所有其他技能在工作前均会读取。您编辑的是文档，而非配置文件。
...
| `commercial-legal:review [文件]` | 自动分类并审查一般服务、委托、评估、公共部门背景、供应商、保密或SaaS合同 |
...
| 技能 | 用途 |
|---|---|
| **cold-start-interview** | 首次运行访谈，写入 `~/.codex/plugins/config/claude-for-legal-zh/commercial-legal/PRACTICE.md` |
| **vendor-agreement-review** | 完整合同手册对合同偏差分析，附修订文本 |
| **service-engagement-review** | 一般服务、委托与评估合同内部审查层：范围、成果、付款、验收及争议路径 |
| **public-sector-contract-overlay** | 公共部门背景内部叠加层：权限、程序、审计留痕及可能的行政协议属性 |
...
您的配置存储于 `~/.codex/plugins/config/claude-for-legal-zh/commercial-legal/PRACTICE.md` 并在插件更新后继续生效。
...
输出：合同分类和适用审查层、逐项风险分析备忘录、待补材料清单、具体修改建议和指定审批人。
...
您在 `~/.codex/plugins/config/claude-for-legal-zh/commercial-legal/PRACTICE.md` 中的实务画像并非一成不变——它会随着您使用插件而持续改进。技能会告知您何时某次输出使用了应予调整的默认值。`playbook-monitor` 代理在您的实务操作偏离合同手册时提议更新。您可以重新运行设置、直接编辑文件或告知某项技能记录新的立场。
...
├── .mcp.json
├── PRACTICE.md                    # 您的团队实务画像 —— 由冷启动访谈撰写，由您编辑
├── README.md
├── agents/
│   ├── renewal-watcher.md
│   ├── deal-debrief.md
